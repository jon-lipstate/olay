...
// Size of the window (in pixels) used for 1-dimensional anti-aliasing along
// each rays.
//   0 - no anti-aliasing
...
vec2 rotate(vec2 v) { return vec2(v.y, -v.x); }
...
void main() {
  ...
  for (int i = 0; i < glyph.count; i++) {
    ...
    if (enableSuperSamplingAntiAliasing) {
      alpha += computeCoverage(inverseDiameter.y, rotate(p0), rotate(p1),
                               rotate(p2));
    }
  }
